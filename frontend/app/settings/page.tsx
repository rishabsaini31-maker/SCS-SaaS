...
export default function SettingsPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto py-2">
      {/* Page Title */}
      <div className="mb-8">
        <h2 className="font-display-sm text-display-sm text-on-surface">Settings</h2>
        <p className="font-body-md text-on-surface-variant mt-1">Manage your business profile, staff access, and platform preferences.</p>
      </div>
        ...
        <div className="flex space-x-8 overflow-x-auto hide-scrollbar">
          {tabs.map((tab) => (
            <button
              ...
              onClick={() => {
                setActiveTab(tab);
                setLastAction(`Switched tab to ${tab}`);
              }}
              type="button"
            >
              {tab}
              {activeTab === tab ? (
                <div className="absolute bottom-[-1px] left-0 right-0 h-[2px] bg-[#0053db]"></div>
              ) : null}
            </button>
          ))}
        </div>
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* Notifications */}
          <div className="lg:col-span-1 bg-white rounded-xl border border-[#E2E8F0] p-6 space-y-6">
