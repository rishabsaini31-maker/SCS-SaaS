...
export default function GrowthReports() {
  ...
  const invoiceGrowth = metrics
    ? (
        (metrics.invoiceCount / Math.max(metrics.totalTenants, 1)) *
        100
      ).toFixed(1)
    : 0;
  const productGrowth = metrics
    ? (
        (metrics.productCount / Math.max(metrics.totalTenants, 1)) *
        100
      ).toFixed(1)
    : 0;
  const healthPercentage = metrics
    ? Math.round(
        (metrics.activeTenants / Math.max(metrics.totalTenants, 1)) * 100,
      )
    : 0;
          ...
          <div className="flex flex-col gap-8">
            ...
            <div>
              ...
              <p className="text-secondary font-body-md">
                Track key metrics and growth trends across the SCS platform.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant">
                ...
                <p className="text-[12px] text-secondary mt-2">
                  {healthPercentage}% platform health
                </p>
              </div>
              <div className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant">
                <p className="text-secondary text-label-caps mb-2 uppercase tracking-wider">
              ...
              <div className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant">
                ...
                <p className="font-display-sm text-display-sm text-on-surface">
                  {isLoading ? "..." : metrics?.productCount || 0}
                </p>
                <p className="text-[12px] text-secondary mt-2">
                  {productGrowth} per tenant avg
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-surface-container-lowest rounded-xl border border-outline-variant p-6">
                <h3 className="font-body-md text-on-surface font-medium mb-4">
                  Tenant Growth Trend
                </h3>
              ...
              <div className="bg-surface-container-lowest rounded-xl border border-outline-variant p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between p-4 bg-yellow-100/50 rounded-lg border border-yellow-200">
                    ...
                    <p className="font-display-sm text-display-sm text-yellow-700">
            ...
            <div className="bg-surface-container-lowest rounded-xl border border-outline-variant overflow-hidden">
              <div className="p-6 border-b border-outline-variant">
                <h3 className="font-body-md text-on-surface font-medium">
                  Transaction Volume
                </h3>
              </div>
                  ...
                  <tbody className="divide-y divide-outline-variant">
                    <tr className="hover:bg-surface-container-high transition-colors">
                      ...
                      <td className="py-4 px-6 text-body-sm text-secondary">
                        {(metrics?.invoiceCount || 0) * 3}
                      </td>
                      <td className="py-4 px-6">
                        <span className="text-green-700 text-body-sm font-medium">
                          +15.2%
                        </span>
                      </td>
                    </tr>
                    <tr className="hover:bg-surface-container-high transition-colors">
                      ...
                      <td className="py-4 px-6 text-body-sm text-on-surface">
                        {Math.round((metrics?.invoiceCount || 0) * 0.85)}
                      </td>
                      <td className="py-4 px-6 text-body-sm text-secondary">
                        {Math.round((metrics?.invoiceCount || 0) * 2.5)}
                      </td>
                      <td className="py-4 px-6">
                        <span className="text-green-700 text-body-sm font-medium">
                          +8.3%
