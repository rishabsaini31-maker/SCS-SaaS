...
export const getCustomerLedger = async (
  ...
  try {
    ...
    const result = await service.getCustomerLedger(customerId, req.tenantId);
    res.json(result);
  } catch (err) {
    next(err);
  }
...
export const getSupplierLedger = async (
  ...
  try {
    ...
    const result = await service.getSupplierLedger(supplierId, req.tenantId);
    res.json(result);
  } catch (err) {
    next(err);
  }
...
export const listAllLedgers = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
    ...
    const result = await service.getAllLedgers(
      {
        entityType: req.query.entityType
          ? Array.isArray(req.query.entityType)
            ? (req.query.entityType[0] as "customer" | "supplier")
            : (req.query.entityType as "customer" | "supplier")
          : undefined,
      },
      req.tenantId,
    );
