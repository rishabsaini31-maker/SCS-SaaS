...
import { createCustomerSchema, updateCustomerSchema, customerIdSchema } from "./customers.schema";
...
export const update = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const customer = await service.updateCustomer(id, data);
    res.json(customer);
  } catch (err) { next(err); }
};
...
export const getLedger = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = customerIdSchema.parse(req.params);
    const result = await service.getCustomerLedger(id);
    res.json(result);
  } catch (err) { next(err); }
};
