...
function parseDatabaseUrl(dbUrl) {
  ...
  return {
    ...
    port,
    requiresSsl,
    redacted: `${url.protocol}//${url.username}:<REDACTED>@${host}:${port}${url.pathname}${url.search}`,
  };
}
...
async function main() {
  try {
    ...
    if (!dbUrl) {
      console.error(
        "Usage: set DATABASE_URL env var or pass it as the first arg",
      );
      process.exit(2);
    }
    ...
    if (requiresSsl) {
      try {
        ...
        console.error(
          `❌ TLS handshake to ${host}:${port} failed:`,
          err.message || err,
        );
        process.exit(4);
      }
      ...
      console.log(
        "ℹ️  SSL not required by DATABASE_URL (sslmode not set to require)",
      );
    }

    console.log(
      "If both checks passed but Prisma still fails, the issue may be credentials, database user permissions, or provider-side network allowlisting.",
    );
    process.exit(0);
  } catch (err) {
